...
function buildTopCalFrame() {
    ...
    var calDoc =
        ...
        "<CENTER>" +
        getMonthSelect() +
        "<INPUT id='INtopYear' VALUE='" + calDate.getFullYear() + "'TYPE=TEXT SIZE=4 MAXLENGTH=4 onChange='setYear()' onKeyDown='if (window.event != null && window.event.keyCode == 13) setYear()'>" +
        "</CENTER>" +
        "</TD>" +
        "</TR>" +
        "<TR>" +
...
function setYear() {
    ...
    if (isFourDigitYear(year)) {
        ...
        writeCalendar();
        document.getElementById("INtopYear").blur();
    }
    else {
...
function INwriteCalendar(event,Iwidth, Iheight,IscreenX,IscreenY)
{
	...
	INscreenY = IscreenY;
	//location and size
	document.getElementById("INIDcalendarWindow").style.left= INscreenX;
	document.getElementById("INIDcalendarWindow").style.top= INscreenY;
	...
	document.getElementById("INIDcalendarWindow").style.left= x - 115;//set it in the mouse click location
	document.getElementById("INIDcalendarWindow").style.top= y + 5;
