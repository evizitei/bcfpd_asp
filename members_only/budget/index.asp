...
if Request.QueryString("Delete") <> "" then
	cnnMain.execute("Delete from Budget where nID = " & Request.QueryString("Delete"))
end if
...
if Request.ServerVariables("REQUEST_METHOD") = "POST" then
	...
	rs.Open "Budget", cnnMain, 3, 3
	rs.AddNew 
	rs("FileName") = mySmartUpload.files.item(1).filename
	rs("BudgetYear") = mySmartUpload.Form("BudgetYear")
...
<table width="98%" border="0" cellspacing="0" cellpadding="5" bordercolor="orange">
	<tr>
		<td class="PageTitle">Budget Update</td>
    </tr>
	<tr>
		<td valign="bottom">
...
<p>	
	<%set rs = cnnMain.execute("select * from Budget order by nID Desc")%>
		<table border="1" cellpadding="4" cellspacing="0">
			...
			<tr>
				...
				<td class="AppLabel" align="center"><%=rs("BudgetYear")%></td>
				<td class="AppLabel" align="left"><a target="_blank" href="/aboutbcfpd/budget/<%=GetPageByContent(rs("ContentType"))%>.asp?nid=<%=rs("nID")%>"><%=rs("FileName")%></a></td>
			</tr>
			<% rs.MoveNext
			   Loop
			   rs.Close 
