...
if Request.QueryString("Delete") <> "" then
	cnnMain.execute("Delete from VolunteerOpportunities where nID = " & Request.QueryString("Delete"))
end if
...
if Request.ServerVariables("REQUEST_METHOD") = "POST" then
	...
	rs.Open "VolunteerOpportunities", cnnMain, 3, 3
	rs.AddNew 
	rs("JobTitle") = mySmartUpload.Form("JobTitle")
	rs("JobApplicationContentType") = trim(mySmartUpload.files.item(1).ContentType)
	mySmartUpload.Files.item(1).FileToField rs("JobApplication")
	rs.Update 
	rs.Close
...
<table width="98%" border="0" cellspacing="0" cellpadding="5" bordercolor="orange">
	<tr>
		<td class="PageTitle">Volunteer Opportunities Update</td>
    </tr>
	<tr>
		<td valign="bottom">
...
<p>	
	<%set rs = cnnMain.execute("select * from VolunteerOpportunities order by nID Desc")%>
		<table border="1" cellpadding="4" cellspacing="0">
			...
			<tr>
				...
				<td class="AppLabel" align="center"><a target="_blank" href="/joinbcfpd/volunteer/<%=GetPageByContent(rs("JobApplicationContentType"))%>.asp?nid=<%=rs("nID")%>&amp;Document=JobApplication">view</a></td>
			</tr>
			<% rs.MoveNext
			   Loop
			   rs.Close 
