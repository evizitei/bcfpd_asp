...
' Create a recordset object called rsCategory.
	...
	strNode = "SELECT * FROM emerg_prep_library_category"

' If there is a request to delete, delete the record.
'If Request.Form("Submit") = "Delete Category" Then
	rsCategory.Open "emerg_prep_library_category", cnnMain, 3, 3
...
If Request.Form("Submit") = "Update Existing Category" Then
	rsCategory.Open "SELECT * FROM emerg_prep_library_category WHERE Category_ID =" & Request.Form("Category_ID"), cnnMain,3,3
		For Each Item in Request.Form
...
<% If Request.Form("Submit") = "Add New Category" OR Request.QueryString("Category_ID") <> "" Then
		...
		If Request.QueryString("Category_ID") <> "" Then
		 rsCategory.Open "SELECT * FROM emerg_prep_library_category WHERE Category_ID = " & Request.QueryString("Category_ID"), cnnMain, 3, 3   
		End If
%>
...
<% Else %>
	...
	<%	
		rsCategory.Open "SELECT * FROM emerg_prep_library_category ORDER BY Name ASC" , cnnMain, 3, 3
	%>
	...
	<!-- 11. A table is created that will contain a delete check box, Make Feature/Headline links, and date info.-->
			<table border="0" cellpadding="3" cellspacing="3">
				<tr>
					<td></td>
					<td align="right"><a href="image_list.asp?Edit=<%=Request.QueryString("Edit")%>&amp;Spot_ID=&amp;FullPath=<%=server.URLEncode("c:\websites\bcdmo\emrgprep\Library\Files")%>&amp;Virtual=<%=server.URLEncode("/emrgprep/Library/Files")%>"><img src="/images/members/buttons/files.gif" border="0"></a></td>
				</tr>		 
				<tr>
					<td></td>
					<td>
