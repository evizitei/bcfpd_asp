...
VideoPath = "c:\web sites\bcfdmo\images\photo_gallery\video\" 
...
strNode = "SELECT * FROM Video_Category"

for each item in Request.Form("Delete")
	rsCategory.Open "SELECT Name FROM Video WHERE Video_Category_ID = " & item, cnnMain,3,3 
	if not rsCategory.EOF then
		do while not rsCategory.EOF
			if fs.FileExists(VideoPath & rsCategory("Name")) then 
				fs.DeleteFile(VideoPath & rsCategory("Name"))	
			end if
			rsCategory.MoveNext
		loop
	end if
	...
	strSQL = "DELETE FROM Video_Category WHERE Category_ID = " & item
...
If Request.Form("Submit") = "Update Category" Then
	rsCategory.Open "Video_Category", cnnMain,3,3
	rsCategory.AddNew
...
If Request.Form("Submit") = "Update Existing Category" Then
	rsCategory.Open "SELECT * FROM Video_Category WHERE Category_ID =" & Request.Form("Category_ID"), cnnMain,3,3
		For Each Item in Request.Form
...
<table border="0" width="95%" cellspacing="0" cellpadding="5">
	<tr>
		<td class="PageTitle">Video Categories For Photo Gallery</td>
    </tr>
	<tr>
		<td valign="bottom">
...
<% If Request.Form("Submit") = "Add New Category" OR Request.QueryString("Category_ID") <> "" Then
		...
		If Request.QueryString("Category_ID") <> "" Then
		 rsCategory.Open "SELECT * FROM Video_Category WHERE Category_ID = " & Request.QueryString("Category_ID"), cnnMain, 3, 3   
		End If
%>
...
<% Else %>
	...
	<%	
		rsCategory.Open "SELECT * FROM Video_Category ORDER BY Name ASC" , cnnMain, 3, 3
	%>
				...
				<td>
				    <table border="0" cellpadding="3" cellspacing="3" bordercolor="#f2f2f2">
						...
						<tr>
							...
							<td align="center" class="ContactInfo"><a class="Update" href="image_list.asp?Category_ID=<%=rsCategory("Category_ID")%>">Category Videos</a></td>
						</tr>
