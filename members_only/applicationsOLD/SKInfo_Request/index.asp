...
'If Request.Form("Submit") = "Delete Contact" Then
	rsApplicant.Open "Application_SK", cnnBCFPD, 3, 3
...
<table width="98%" border="0" cellspacing="0" cellpadding="5" bordercolor="orange">
	...
	<%rsApplicant.Open "Select * From Application_SK Order By Namereq ASC", cnnBCFPD, 3 %>
	...
	<!-- 10. A table is created that will contain a delete check box, Headline links, and date info.-->
      <table border="1" cellpadding="3" cellspacing="0" bordercolor="#f2f2f2">
		<tr>
			...
			<td class="subheadercenter">Dept/School</td>
		</tr>
		...
		<tr>
			...
			<td class="contactinfo"><%=rsApplicant("Namereq")%></td>
			<td class="contactinfo"><%=rsApplicant("Title")%></td>
			<td class="contactinfo"><%=rsApplicant("Address1req")%>
				<% if rsApplicant("Address2") <> "" then%>
				<%=	"--" & rsApplicant("Address2") %>
				<% end if%>
			</td>
			<td class="contactinfo"><%=rsApplicant("Cityreq")%></td>
			<td class="contactinfo"><%=rsApplicant("Statereq")%></td>
			<td class="contactinfo"><%=rsApplicant("Zipreq")%></td>
			<td class="contactinfo"><%=rsApplicant("Day_Phonereq")%></td>
			<td class="contactinfo"><%=rsApplicant("Night_Time_Phone")%></td>
			<td class="contactinfo"><%=rsApplicant("Fax")%></td>
			<td class="contactinfo"><%=rsApplicant("Email")%></td>
			<td class="contactinfo"><%=rsApplicant("DeptNamereq")%></td>
		</tr>
		<%	rsApplicant.MoveNext
			Loop
		   	rsApplicant.Close 
