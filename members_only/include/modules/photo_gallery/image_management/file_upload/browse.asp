...
//File must be either a .jpg or .gif to upload to server.
	function Verify() {
		...
		var ext1 = name1.substring(number1, number1 - 3).toUpperCase();

		var name2 = window.document.Browse.file2.value;
		var number2 = name2.length;
		var ext2 = name2.substring(number2, number2 - 3).toUpperCase();
		
		ext1.to
		if((document.all.file1.value == "") || (document.all.file2.value == "")) {
			alert("You must choose 2 files before continuing.");
			//return false;
		}
		...
		else if ((ext2 != 'GIF') && (ext2 != 'JPG') && (ext2 != 'MPG') && (ext2 !='MOV'))  {
			alert("You can't upload *" + ext2 + " files. Only files with .gif or .jpg extensions can be uploaded.");
			//return false;
		}
		else {
		...
		<table cellpadding="3">
		  <tr>
		    <td>
				...
				<p><font size="2">Please choose <b>2</b> files to upload, then click next to continue.</font>
				<font size="1">The Thumbnail image will be the Photo Gallery Image that you want to display. The Full
				Size image will be displayed when user clicks on the thumbnail.</font><p><br>
				<b>Thumbnail:</b>
				<br><input type="file" id="file1" name="file1" size="30">
				<br><br><b>Full Size:</b>
				<br><input type="file" id="file2" name="file2" size="30">
				<!--<input disabled type="file" id="file3" name="file3" size="30">-->
		    </td>
		  </tr>
		  <tr>
		    <td align="center">
